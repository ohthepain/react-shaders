...
const singleLineCommentRegex =
	/\/\/.*[a-zA-Z0-9_$]+\s*=\s*.*|\/\/.*function\s+[a-zA-Z0-9_$]+\s*\(.*\)|\/\/.*if\s*\(.*\)|\/\/.*for\s*\(.*\)|\/\/.*while\s*\(.*\)/;
const multiLineCommentRegex =
	/\/\*[\s\S]*?\*\/.*[a-zA-Z0-9_$]+\s*=\s*.*|\/\*[\s\S]*?\*\/.*function\s+[a-zA-Z0-9_$]+\s*\(.*\)|\/\*[\s\S]*?\*\/.*if\s*\(.*\)|\/\*[\s\S]*?\*\/.*for\s*\(.*\)|\/\*[\s\S]*?\*\/.*while\s*\(.*\)/;
...
const checkFileForComments = (filePath) => {
	...
	rl.on('line', (line) => {
		lineNumber++;
		if (
			singleLineCommentRegex.test(line) ||
			multiLineCommentRegex.test(line)
		) {
			console.log(
				`Commented-out code found in file: ${filePath}:${lineNumber}`,
			);
